...
async function createSignedAdUpload(originalName, mimeType, size) {
  if (!isConfigured || !supabase) return null;
  const maxBytes = 45 * 1024 * 1024;
  if (!Number.isFinite(size) || size <= 0 || size > maxBytes) {
    throw new Error("Ad file must be larger than 0 bytes and no larger than 45 MB.");
  }
  ...
  if (!ext) throw new Error("Unsupported ad file type.");
  const crypto = require("crypto");
  const bucket = process.env.SUPABASE_ADS_BUCKET || "ad-media";
  try {
    const allowedMimeTypes = Object.keys(allowed);
    try { await supabase.storage.createBucket(bucket, { public: true, fileSizeLimit: "45MB", allowedMimeTypes }); } catch (_) {}
    try { await supabase.storage.updateBucket(bucket, { public: true, fileSizeLimit: "45MB", allowedMimeTypes }); } catch (_) {}
    const objectName = `ads/${Date.now()}_${crypto.randomBytes(16).toString("hex")}${ext}`;
    const { data, error } = await supabase.storage.from(bucket).createSignedUploadUrl(objectName, { upsert: false });
    if (error || !data?.signedUrl) {
      console.warn("[SUPABASE] createSignedUploadUrl:", error?.message || "No signed URL");
      return null;
    }
    const publicUrl = supabase.storage.from(bucket).getPublicUrl(objectName).data?.publicUrl || "";
    return { bucket, path: objectName, token: data.token, signedUrl: data.signedUrl, publicUrl, mimeType: mime };
  } catch (err) {
    console.warn("[SUPABASE] createSignedAdUpload exception:", err.message);
    return null;
  }
}
